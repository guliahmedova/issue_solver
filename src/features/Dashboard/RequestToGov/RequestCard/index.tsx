...
export default function ApplyCard() {
  const { isLoading, data } = useRequest(API.organization_requests_all);
  ...
  return (
    <div className="bg-white flex flex-col gap-6 px-4  py-4 rounded-xl border-l-8 border-warning">
      {requestsData &&
        requestsData.map((item: requestsDataType) => {
          return (
            <Fragment key={requestsData.requestId}>
              <div className="w-full flex justify-between items-center ">
                <div className="flex flex-col gap-2 items-start">
                  <div className="flex gap-1 items-center">
                    <Image src={NameIcon} alt="icon" width={24} height={24} />
                    <span className="text-blue-primary text-xs">{requestsData.fullName}</span>
                  </div>
                  <span className="text-surface-secondary text-sm font-medium">
                    ID:{requestsData.requestId}
                  </span>
                </div>
                <h6 className="text-sm font-medium">{requestsData.organizationName}</h6>
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <div className="flex px-5 py-2 gap-2 bg-gray-disabled rounded-[100px]">
                    ...
                    <span className="text-text-gray text-xxs">{requestsData.status}</span>
                  </div>
                  <div className="px-4 py-2  bg-gray-disabled rounded-[100px]">
                    <span className="text-text-gray text-xxs">
                      Küçə heyvanlarına qarşı zorbalıq
                    </span>
                  </div>
                </div>
                <p className="text-parag-gray text-sm font-medium">{requestsData.description}</p>
              </div>
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <Image src={LocationIcon} alt="location" width={20} height={20} />
                  <span className="text-sm-alt text-blue-primary">{requestsData.adress}</span>
                </div>
                <div className="flex gap-2">
                  <Image src={CalendarIcon} alt="calendar" width={20} height={20} />
                  <span className="text-sm-alt ">{requestsData.createDate}</span>
                </div>
              </div>
              <Divider />
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-6 items-center">
                  <div className="flex flex-col gap-1 items-center">
                    ...
                    <span>{requestsData.likeCount}</span>
                  </div>
                  <div className="flex flex-col gap-1 items-center">
                    <button>
                      <Image src={CommentIcon} alt="comment" width={20} height={20} />
                    </button>
                    <span>{requestsData.commentCount}</span>
                  </div>
                </div>
              </div>
            </Fragment>
